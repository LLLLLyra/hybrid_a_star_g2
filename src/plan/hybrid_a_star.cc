...
HybridAStar::HybridAStar(
  ...
  reed_shepp_generator_ =
      std::make_unique<ReedSheppG2>(vehicle_param_, planner_open_space_config_);
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator_clothoid(
    std::shared_ptr<Node3d> current_node, size_t next_node_index) {
  double steering = 0.0;
  double traveled_distance = 0.0;
  if (next_node_index < static_cast<double>(next_node_num_) / 2) {
  ...
  intermediate_y.push_back(last_y);
  intermediate_phi.push_back(last_phi);
  double current_kappa = current_node->GetKappa();
  double kappa = 0.0;
  bool circled = std::abs(std::abs(current_kappa) - max_kappa_) < 1e-3 &&
                 std::abs(std::abs(steering) - max_steer_angle_) < 1e-3;
  bool linear = std::abs(current_kappa) < 1e-3 && std::abs(steering) < 1e-3;
  bool left = steering > 0;

  if (circled) {
    const double angle = traveled_distance * current_kappa;
    const double theta = std::atan2(current_node->GetY(), current_node->GetX());
    const double x0 = current_node->GetX() - std::cos(theta) / current_kappa;
    const double y0 = current_node->GetY() - std::sin(theta) / current_kappa;
    kappa = current_kappa;
    for (size_t i = 0; i < 3; i++) {
      const double next_x = x0 + std::cos(left ? theta + angle * (i + 1)
                                               : theta - angle * (i + 1));
      const double next_y = y0 + std::cos(left ? theta + angle * (i + 1)
                                               : theta - angle * (i + 1));
      const double next_phi =
          std::atan2(next_y, next_x) + (left ? M_PI_2 : -M_PI_2);
      intermediate_x.push_back(next_x);
      intermediate_y.push_back(next_y);
      intermediate_phi.push_back(next_phi);
    }
    ...
    kappa = current_kappa;
    for (size_t i = 0; i < 3; i++) {
      const double next_x = last_x + std::cos(last_phi) * traveled_distance;
      const double next_y = last_y + std::sin(last_phi) * traveled_distance;
      intermediate_x.push_back(next_x);
      intermediate_y.push_back(next_y);
      intermediate_phi.push_back(last_phi);
      last_x = next_x;
    ...
    const double A =
        (left ? (max_kappa_ - current_kappa) : (-max_kappa_ - current_kappa)) /
        traveled_distance / 3.0 *
        ((std::abs(steering) + 1e-3) / max_steer_angle_);
    for (size_t i = 0; i < 3; i++) {
      current_kappa += A * traveled_distance;
      const double next_phi = last_phi + current_kappa * traveled_distance;
      const double next_x = last_x + std::cos(next_phi) * traveled_distance;
      const double next_y = last_y + std::sin(next_phi) * traveled_distance;
      intermediate_x.push_back(next_x);
      intermediate_y.push_back(next_y);
      intermediate_phi.push_back(next_phi);
      last_x = next_x;
...
bool HybridAStar::Plan(
  ...
  while (!open_pq_.empty()) {
    ...
    for (size_t i = 0; i < next_node_num_; ++i) {
      std::shared_ptr<Node3d> next_node =
          Next_node_generator_clothoid(current_node, i);
